...
#define LOG_DOMAIN modem_ublox_sara_r41xx
...
static const struct mdm_control_pinconfig pinconfig[] = {
	/* MDM_POWER */
	PINCONFIG(DT_UBLOX_SARA_R41XX_0_MDM_POWER_GPIOS_GPIO_CONTROLLER,
		  DT_UBLOX_SARA_R41XX_0_MDM_POWER_GPIOS_GPIO_PIN),

	/* MDM_RESET */
	PINCONFIG(DT_UBLOX_SARA_R41XX_0_MDM_RESET_GPIOS_GPIO_CONTROLLER,
		  DT_UBLOX_SARA_R41XX_0_MDM_RESET_GPIOS_GPIO_PIN),
};

#define MDM_UART_DEV_NAME		DT_UBLOX_SARA_R41XX_0_BUS_NAME
...
K_THREAD_STACK_DEFINE(modem_rx_stack,
		       CONFIG_MODEM_UBLOX_SARA_R41XX_RX_STACK_SIZE);
struct k_thread modem_rx_thread;

/* RX thread work queue */
K_THREAD_STACK_DEFINE(modem_workq_stack,
		      CONFIG_MODEM_UBLOX_SARA_R41XX_RX_WORKQ_STACK_SIZE);
...
restart:
	...
	ret = send_at_cmd(NULL, "AT+CGDCONT=1,\"IP\",\""
				CONFIG_MODEM_UBLOX_SARA_R41XX_OPERATOR "\"",
			  MDM_CMD_TIMEOUT);
	if (ret < 0) {
...
NET_DEVICE_OFFLOAD_INIT(modem_sara_r41xx, "MODEM_SARA_R41XX",
			modem_init, &ictx,
			NULL, CONFIG_MODEM_UBLOX_SARA_R41XX_INIT_PRIORITY, &api_funcs,
			MDM_MAX_DATA_LENGTH);
